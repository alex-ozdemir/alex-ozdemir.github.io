...
This CDF shows you, for a given number of `unsafe` functions + blocks, what
percent of crates use that many or fewer. As an example, the line crosses 80%
at ~3 funcations + blocks, so 80% of crates use 3 or fewer `unsafe` functions
and blocks (also notice that the horizontal axis is logarithmic).
